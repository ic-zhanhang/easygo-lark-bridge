 ...
 * 仅服务非「小组」的 Group Topic Only 入口，避免 Agent 去翻其它群旁观日志。
 */
...
export function isolationBanner(chatId: string, threadId?: string): string {
	...
	return [
		...
		"本请求只服务当前群/话题。禁止读取其它群的聊天记录，尤其禁止：",
		XIAOZHU_FORBIDDEN.map((p) => `  - ${p}`).join("\n"),
		"上述路径仅「小组」会话可用。上下文不足时，以本 prompt 注入的当前话题消息为准；不要用 memory-tool / 文档/小组* 顶替。",
	].join("\n");
}
